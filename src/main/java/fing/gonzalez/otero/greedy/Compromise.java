...
class Compromise {
    ...
    public Compromise    (int variables, int vehicles) {
    ...
    private Pair<Double, Integer> calculateDeltaAndBestPosition(int receptor, List<Integer> route){
        ...
        double delta, routeC = routeCost(route), routeT = routeTime(route);
        ...
        for (int pos = 0; pos <= newRoute.size(); pos++) {
            ...
            delta = .5*(routeCost(newRoute) - routeC)/routeC  + .5*(routeTime(newRoute) - routeT)/routeT; // costo, tiempo
    ...
    private double routeCost(List<Integer> route) {
        ...
        int pre = 0;
        for (Integer id: route) {
            totalCost += obtainCost(pre,id);
            pre = id;
        }
        totalCost += obtainCost(pre, 0);
        return totalCost;
    }
